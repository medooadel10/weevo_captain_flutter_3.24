...
class CarInformation extends StatefulWidget {
  static const String id = 'Car_Information';
...
class _CarInformationState extends State<CarInformation> {
  ...
  void initState() {
    ...
    _carPlateController.text = _authProvider.courierVehicleNumber!;
    _deliveryMethodController.text =
        _authProvider.courierDeliveryMethod == 'truck'
            ? 'سيارة شحن'
            : _authProvider.courierDeliveryMethod == 'bicycle'
                ? 'دراجة'
                : _authProvider.courierDeliveryMethod == 'motorbike'
                    ? 'دراجة نارية'
                    : _authProvider.courierDeliveryMethod == 'car'
                        ? 'سيارة ملاكي'
                        : _authProvider.courierDeliveryMethod == 'none'
                            ? 'اخري'
                            : '';
    _modelNameController.text = _authProvider.courierVehicleModel!;
    _colorController.text = _authProvider.courierVehicleColor!;
                      ...
                      child: Column(
                        children: [
                          EditText(
                            ...
                            upperTitle: true,
                            shouldDisappear: !_deliveryMethodFocused &&
                                !_deliveryMethodEmpty,
  ...
  Future<void> update(AuthProvider authProvider) async {
    showDialog(
        context: context,
        barrierDismissible: false,
        builder: (context) => const Loading());
    await authProvider.courierCarInformation(
      vehicleMethod: _deliveryMethod == 'سيارة ملاكي'
          ? 'car'
    ...
    if (authProvider.carInfoState == NetworkState.success) {
      Navigator.pop(navigator.currentContext!);
      showDialog(
        context: navigator.currentContext!,
        builder: (context) => ActionDialog(
          content: 'تم تغيير معلومات المركبة بنجاح',
          onCancelClick: () {
            Navigator.pop(context);
            Navigator.pop(context);
          },
    ...
    } else if (authProvider.carInfoState == NetworkState.error) {
      Navigator.pop(navigator.currentContext!);
      showDialog(
        context: navigator.currentContext!,
        builder: (context) => ActionDialog(
          content: ' حدث خطاًبرجاء المحاولة مرة اخري',
          onCancelClick: () {
            Navigator.pop(context);
          },
          cancelAction: 'حسناً',
    ...
    } else if (authProvider.carInfoState == NetworkState.logout) {
      check(
          ctx: navigator.currentContext!,
          auth: authProvider,
          state: authProvider.carInfoState!);
    }
  }
